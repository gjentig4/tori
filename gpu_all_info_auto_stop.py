    ...
    if response.status_code == 200:
        ...
        for link in links:
            ...
            if full_link in processed_links:
                print(f"Stopping as reached previously processed link: {full_link}")
                return items_details  # Return the items processed so far
...
item_details_list = scrape_page(base_url, processed_links)  
